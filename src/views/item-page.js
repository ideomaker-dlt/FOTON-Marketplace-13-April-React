import React from 'react'

import { Helmet } from 'react-helmet'

import LayoutWithSidebar from '../components/layout-with-sidebar'
import './item-page.css'

const ItemPage = (props) => {
  return (
    <div className="item-page-container">
      <Helmet>
        <title>
          Item-Page - FOTON Marketplace for NFTs and Digital Collectibles on
          Radix DLT
        </title>
        <meta
          name="description"
          content="Collect , Trade or Mint NFTs and find your Communities"
        />
        <meta
          property="og:title"
          content="Item-Page - FOTON Marketplace for NFTs and Digital Collectibles on Radix DLT"
        />
        <meta
          property="og:description"
          content="Collect , Trade or Mint NFTs and find your Communities"
        />
      </Helmet>
      <LayoutWithSidebar rootClassName="layout-with-sidebar-root-class-name14">
        <div className="item-page-wrapper-all">
          <div className="item-page-container-left">
            <div className="item-page-item-image">
              <img
                alt="image"
                src="/playground_assets/vlad__foton.is__radix.stream_pirates_tropical_island_illustrati_766cf345-d799-426c-ac63-a659abb90994-1500w.png"
                className="item-page-image"
              />
            </div>
          </div>
          <div className="item-page-container-right">
            <div className="item-page-item-info-box">
              <div className="item-page-name-item">
                <div className="item-page-container01">
                  <h1 className="item-page-text">Item name #2044</h1>
                  <div className="item-page-container02">
                    <p className="item-page-text01">
                      <span>By Smooth Collective</span>
                      <br></br>
                    </p>
                    <p className="item-page-text04">
                      <span>Verified</span>
                      <br></br>
                    </p>
                  </div>
                </div>
              </div>
              <div className="item-page-container03">
                <div className="item-page-description-collection">
                  <div className="item-page-container04">
                    <h2 className="item-page-text07">
                      <span>
                        The crew of pirates gathered around a campfire on a
                        tropical island, their ship anchored offshore. They
                        toasted their recent plunder and shared stories of
                        adventure. The sound of waves and the scent of saltwater
                        filled the air, as they planned their next raid under
                        the glow of the moon.
                      </span>
                      <br></br>
                    </h2>
                    <p className="item-page-text10">
                      <span>
                        Created on 23 Apr 2023
                        <span
                          dangerouslySetInnerHTML={{
                            __html: ' ',
                          }}
                        />
                      </span>
                      <br></br>
                    </p>
                  </div>
                </div>
              </div>
              <div className="item-page-small-stats">
                <div className="item-page-small-card">
                  <span className="item-page-text13">10000</span>
                  <span className="item-page-text14">Items</span>
                </div>
                <div className="item-page-small-card1">
                  <span className="item-page-text15">573</span>
                  <span className="item-page-text16">Followers</span>
                </div>
                <div className="item-page-small-card2">
                  <span className="item-page-text17">28k</span>
                  <span className="item-page-text18">Views</span>
                </div>
                <div className="item-page-small-card3">
                  <span className="item-page-text19">62</span>
                  <span className="item-page-text20">Days to Drop</span>
                </div>
              </div>
              <div className="item-page-container05">
                <button type="button" className="item-page-button">
                  <svg
                    viewBox="0 0 1024 1024"
                    className="item-page-icon fill-color-white"
                  >
                    <path d="M512 658l160 96-42-182 142-124-188-16-72-172-72 172-188 16 142 124-42 182zM938 394l-232 202 70 300-264-160-264 160 70-300-232-202 306-26 120-282 120 282z"></path>
                  </svg>
                  <span className="item-page-text21">
                    <span>Follow</span>
                    <br></br>
                  </span>
                </button>
                <button type="button" className="item-page-button01">
                  <svg
                    viewBox="0 0 1024 1024"
                    className="item-page-icon02 fill-color-gray"
                  >
                    <path d="M854 768v-384h-172v384h172zM640 554v-170h-470v170h470zM640 768v-170h-470v170h470zM854 170q34 0 59 26t25 60v512q0 34-25 60t-59 26h-684q-34 0-59-26t-25-60v-512q0-34 25-60t59-26h684z"></path>
                  </svg>
                  <span className="item-page-text24">
                    <span>Follow</span>
                    <br></br>
                  </span>
                </button>
                <button type="button" className="item-page-button02">
                  <svg
                    viewBox="0 0 950.8571428571428 1024"
                    className="item-page-icon04 fill-color-gray"
                  >
                    <path d="M925.714 233.143c-25.143 36.571-56.571 69.143-92.571 95.429 0.571 8 0.571 16 0.571 24 0 244-185.714 525.143-525.143 525.143-104.571 0-201.714-30.286-283.429-82.857 14.857 1.714 29.143 2.286 44.571 2.286 86.286 0 165.714-29.143 229.143-78.857-81.143-1.714-149.143-54.857-172.571-128 11.429 1.714 22.857 2.857 34.857 2.857 16.571 0 33.143-2.286 48.571-6.286-84.571-17.143-148-91.429-148-181.143v-2.286c24.571 13.714 53.143 22.286 83.429 23.429-49.714-33.143-82.286-89.714-82.286-153.714 0-34.286 9.143-65.714 25.143-93.143 90.857 112 227.429 185.143 380.571 193.143-2.857-13.714-4.571-28-4.571-42.286 0-101.714 82.286-184.571 184.571-184.571 53.143 0 101.143 22.286 134.857 58.286 41.714-8 81.714-23.429 117.143-44.571-13.714 42.857-42.857 78.857-81.143 101.714 37.143-4 73.143-14.286 106.286-28.571z"></path>
                  </svg>
                  <span className="item-page-text27">
                    <span>Follow</span>
                    <br></br>
                  </span>
                </button>
                <button type="button" className="item-page-button03">
                  <span className="item-page-text30">
                    <span>Follow</span>
                    <br></br>
                  </span>
                  <svg
                    viewBox="0 0 1024 1024"
                    className="item-page-icon06 fill-color-gray"
                  >
                    <path d="M512 0c-282.8 0-512 229.2-512 512s229.2 512 512 512 512-229.2 512-512-229.2-512-512-512zM763.6 351l-84 395.8c-5.8 28.2-22.8 34.8-46.4 21.8l-128-94.6-61.4 59.8c-7.2 7-12.8 12.8-25.6 12.8-16.6 0-13.8-6.2-19.4-22l-43.6-143.2-126.6-39.4c-27.4-8.4-27.6-27.2 6.2-40.6l493.2-190.4c22.4-10.2 44.2 5.4 35.6 40z"></path>
                  </svg>
                </button>
                <button type="button" className="item-page-button04">
                  <span className="item-page-text33">
                    <span>Follow</span>
                    <br></br>
                  </span>
                  <svg
                    viewBox="0 0 877.7142857142857 1024"
                    className="item-page-icon08 fill-color-gray"
                  >
                    <path d="M731.429 682.857c0-67.429-54.857-122.286-121.714-122.286-32 0-61.143 12.571-82.857 33.143l-137.714-68.571c0.571-4.571 1.143-8.571 1.143-13.143s-0.571-8.571-1.143-13.143l137.714-68.571c21.714 20.571 50.857 33.143 82.857 33.143 66.857 0 121.714-54.857 121.714-122.286 0-66.857-54.857-121.714-121.714-121.714-67.429 0-122.286 54.857-122.286 121.714 0 4.571 0.571 8.571 1.143 13.143l-137.714 68.571c-21.714-20-50.857-32.571-82.857-32.571-66.857 0-121.714 54.286-121.714 121.714s54.857 121.714 121.714 121.714c32 0 61.143-12.571 82.857-32.571l137.714 68.571c-0.571 4.571-1.143 8.571-1.143 13.143 0 66.857 54.857 121.714 122.286 121.714 66.857 0 121.714-54.857 121.714-121.714zM877.714 237.714v548.571c0 90.857-73.714 164.571-164.571 164.571h-548.571c-90.857 0-164.571-73.714-164.571-164.571v-548.571c0-90.857 73.714-164.571 164.571-164.571h548.571c90.857 0 164.571 73.714 164.571 164.571z"></path>
                  </svg>
                </button>
              </div>
            </div>
          </div>
        </div>
        <div className="item-page-container06">
          <div className="item-page-item-image1">
            <img
              alt="image"
              src="/playground_assets/vlad__foton.is__radix.stream_pirates_tropical_island_illustrati_766cf345-d799-426c-ac63-a659abb90994-1500w.png"
              className="item-page-image1"
            />
          </div>
          <div className="item-page-collection-info-box">
            <div className="item-page-container07">
              <div className="item-page-avatar-logo">
                <img
                  src="/playground_assets/vlad__foton.is__radix.stream_pirate_crew_member_illustration_352025ba-0bac-4b8e-9a47-9d8da5bb2ceb-200h.png"
                  className="item-page-image2"
                />
              </div>
              <div className="item-page-container08">
                <h1 className="item-page-text36">Collection One</h1>
                <div className="item-page-container09">
                  <p className="item-page-text37">
                    <span>By Smooth Collective</span>
                    <br></br>
                  </p>
                  <p className="item-page-text40">
                    <span>Verified</span>
                    <br></br>
                  </p>
                </div>
              </div>
            </div>
            <div className="item-page-container10">
              <div className="item-page-description-collection1">
                <div className="item-page-container11">
                  <h2 className="item-page-text43">
                    <span>
                      The crew of pirates gathered around a campfire on a
                      tropical island, their ship anchored offshore. They
                      toasted their recent plunder and shared stories of
                      adventure. The sound of waves and the scent of saltwater
                      filled the air, as they planned their next raid under the
                      glow of the moon.
                    </span>
                    <br></br>
                  </h2>
                  <div className="item-page-container12"></div>
                  <p className="item-page-text46">
                    <span>
                      Created on 23 Apr 2023
                      <span
                        dangerouslySetInnerHTML={{
                          __html: ' ',
                        }}
                      />
                    </span>
                    <br></br>
                  </p>
                </div>
              </div>
            </div>
            <div className="item-page-small-stats1">
              <div className="item-page-small-card4">
                <span className="item-page-text49">10000</span>
                <span className="item-page-text50">Items</span>
              </div>
              <div className="item-page-small-card5">
                <span className="item-page-text51">573</span>
                <span className="item-page-text52">Followers</span>
              </div>
              <div className="item-page-small-card6">
                <span className="item-page-text53">28k</span>
                <span className="item-page-text54">Views</span>
              </div>
              <div className="item-page-small-card7">
                <span className="item-page-text55">62</span>
                <span className="item-page-text56">Days to Drop</span>
              </div>
            </div>
            <div className="item-page-container13">
              <button type="button" className="item-page-button05">
                <svg
                  viewBox="0 0 1024 1024"
                  className="item-page-icon10 fill-color-white"
                >
                  <path d="M512 658l160 96-42-182 142-124-188-16-72-172-72 172-188 16 142 124-42 182zM938 394l-232 202 70 300-264-160-264 160 70-300-232-202 306-26 120-282 120 282z"></path>
                </svg>
                <span className="item-page-text57">
                  <span>Follow</span>
                  <br></br>
                </span>
              </button>
              <button type="button" className="item-page-button06">
                <svg
                  viewBox="0 0 1024 1024"
                  className="item-page-icon12 fill-color-gray"
                >
                  <path d="M854 768v-384h-172v384h172zM640 554v-170h-470v170h470zM640 768v-170h-470v170h470zM854 170q34 0 59 26t25 60v512q0 34-25 60t-59 26h-684q-34 0-59-26t-25-60v-512q0-34 25-60t59-26h684z"></path>
                </svg>
                <span className="item-page-text60">
                  <span>Follow</span>
                  <br></br>
                </span>
              </button>
              <button type="button" className="item-page-button07">
                <svg
                  viewBox="0 0 950.8571428571428 1024"
                  className="item-page-icon14 fill-color-gray"
                >
                  <path d="M925.714 233.143c-25.143 36.571-56.571 69.143-92.571 95.429 0.571 8 0.571 16 0.571 24 0 244-185.714 525.143-525.143 525.143-104.571 0-201.714-30.286-283.429-82.857 14.857 1.714 29.143 2.286 44.571 2.286 86.286 0 165.714-29.143 229.143-78.857-81.143-1.714-149.143-54.857-172.571-128 11.429 1.714 22.857 2.857 34.857 2.857 16.571 0 33.143-2.286 48.571-6.286-84.571-17.143-148-91.429-148-181.143v-2.286c24.571 13.714 53.143 22.286 83.429 23.429-49.714-33.143-82.286-89.714-82.286-153.714 0-34.286 9.143-65.714 25.143-93.143 90.857 112 227.429 185.143 380.571 193.143-2.857-13.714-4.571-28-4.571-42.286 0-101.714 82.286-184.571 184.571-184.571 53.143 0 101.143 22.286 134.857 58.286 41.714-8 81.714-23.429 117.143-44.571-13.714 42.857-42.857 78.857-81.143 101.714 37.143-4 73.143-14.286 106.286-28.571z"></path>
                </svg>
                <span className="item-page-text63">
                  <span>Follow</span>
                  <br></br>
                </span>
              </button>
              <button type="button" className="item-page-button08">
                <span className="item-page-text66">
                  <span>Follow</span>
                  <br></br>
                </span>
                <svg
                  viewBox="0 0 1024 1024"
                  className="item-page-icon16 fill-color-gray"
                >
                  <path d="M512 0c-282.8 0-512 229.2-512 512s229.2 512 512 512 512-229.2 512-512-229.2-512-512-512zM763.6 351l-84 395.8c-5.8 28.2-22.8 34.8-46.4 21.8l-128-94.6-61.4 59.8c-7.2 7-12.8 12.8-25.6 12.8-16.6 0-13.8-6.2-19.4-22l-43.6-143.2-126.6-39.4c-27.4-8.4-27.6-27.2 6.2-40.6l493.2-190.4c22.4-10.2 44.2 5.4 35.6 40z"></path>
                </svg>
              </button>
              <button type="button" className="item-page-button09">
                <span className="item-page-text69">
                  <span>Follow</span>
                  <br></br>
                </span>
                <svg
                  viewBox="0 0 877.7142857142857 1024"
                  className="item-page-icon18 fill-color-gray"
                >
                  <path d="M731.429 682.857c0-67.429-54.857-122.286-121.714-122.286-32 0-61.143 12.571-82.857 33.143l-137.714-68.571c0.571-4.571 1.143-8.571 1.143-13.143s-0.571-8.571-1.143-13.143l137.714-68.571c21.714 20.571 50.857 33.143 82.857 33.143 66.857 0 121.714-54.857 121.714-122.286 0-66.857-54.857-121.714-121.714-121.714-67.429 0-122.286 54.857-122.286 121.714 0 4.571 0.571 8.571 1.143 13.143l-137.714 68.571c-21.714-20-50.857-32.571-82.857-32.571-66.857 0-121.714 54.286-121.714 121.714s54.857 121.714 121.714 121.714c32 0 61.143-12.571 82.857-32.571l137.714 68.571c-0.571 4.571-1.143 8.571-1.143 13.143 0 66.857 54.857 121.714 122.286 121.714 66.857 0 121.714-54.857 121.714-121.714zM877.714 237.714v548.571c0 90.857-73.714 164.571-164.571 164.571h-548.571c-90.857 0-164.571-73.714-164.571-164.571v-548.571c0-90.857 73.714-164.571 164.571-164.571h548.571c90.857 0 164.571 73.714 164.571 164.571z"></path>
                </svg>
              </button>
            </div>
          </div>
        </div>
        <div className="item-page-tabs-row">
          <span className="item-page-text72">Items</span>
          <span className="item-page-text73">About</span>
          <span className="item-page-text74">Followers</span>
        </div>
        <div className="item-page-container14"></div>
      </LayoutWithSidebar>
    </div>
  )
}

export default ItemPage
