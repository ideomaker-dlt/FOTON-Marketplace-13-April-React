import React from 'react'

import PropTypes from 'prop-types'

import './ranking-preview1.css'

const RankingPreview1 = (props) => {
  return (
    <div className={`ranking-preview1-ranking-preview ${props.rootClassName} `}>
      <div className="ranking-preview1-categories">
        <div className="ranking-preview1-container">
          <div className="ranking-preview1-top">
            <span className="ranking-preview1-text BigTabs">
              <span className="">Top</span>
              <br className=""></br>
            </span>
          </div>
          <div className="ranking-preview1-newest">
            <span className="ranking-preview1-text03 BigTabs">
              <span className="">Newest</span>
              <br className=""></br>
            </span>
          </div>
        </div>
        <div className="ranking-preview1-filter-buttons">
          <div className="ranking-preview1-timeframe">
            <button className="ranking-preview1-button">
              <span className="ranking-preview1-text06">
                <span className="">1h</span>
                <br className=""></br>
              </span>
            </button>
            <button className="ranking-preview1-button1">
              <span className="ranking-preview1-text09">
                <span className="">6h</span>
                <br className=""></br>
              </span>
            </button>
            <button className="ranking-preview1-button2">
              <span className="ranking-preview1-text12">
                <span className="">24h</span>
                <br className=""></br>
              </span>
            </button>
            <button className="ranking-preview1-button3">
              <span className="ranking-preview1-text15">
                <span className="">7d</span>
                <br className=""></br>
              </span>
            </button>
          </div>
          <div
            data-thq="thq-dropdown"
            className="ranking-preview1-category list-item"
          >
            <div
              data-thq="thq-dropdown-toggle"
              className="ranking-preview1-dropdown-toggle"
            >
              <span className="ranking-preview1-text18">Category</span>
              <div
                data-thq="thq-dropdown-arrow"
                className="ranking-preview1-dropdown-arrow"
              >
                <svg viewBox="0 0 1024 1024" className="ranking-preview1-icon">
                  <path
                    d="M316 366l196 196 196-196 60 60-256 256-256-256z"
                    className=""
                  ></path>
                </svg>
              </div>
            </div>
            <ul
              data-thq="thq-dropdown-list"
              className="ranking-preview1-dropdown-list"
            >
              <li
                data-thq="thq-dropdown"
                className="ranking-preview1-dropdown list-item"
              >
                <div
                  data-thq="thq-dropdown-toggle"
                  className="ranking-preview1-dropdown-toggle1"
                >
                  <span className="ranking-preview1-text19">Avatars</span>
                </div>
              </li>
              <li
                data-thq="thq-dropdown"
                className="ranking-preview1-dropdown1 list-item"
              >
                <div
                  data-thq="thq-dropdown-toggle"
                  className="ranking-preview1-dropdown-toggle2"
                >
                  <span className="ranking-preview1-text20">
                    {props.text58}
                  </span>
                </div>
                <div
                  data-thq="thq-dropdown-toggle"
                  className="ranking-preview1-dropdown-toggle3"
                >
                  <span className="ranking-preview1-text21">
                    {props.text59}
                  </span>
                </div>
              </li>
              <li
                data-thq="thq-dropdown"
                className="ranking-preview1-dropdown2 list-item"
              >
                <div
                  data-thq="thq-dropdown-toggle"
                  className="ranking-preview1-dropdown-toggle4"
                >
                  <span className="ranking-preview1-text22">Memberships</span>
                </div>
              </li>
            </ul>
          </div>
          <span className="ranking-preview1-text23 button">View All</span>
        </div>
      </div>
      <div className="ranking-preview1-top-collections">
        <div className="ranking-preview1-header-table">
          <div className="header-first-column">
            <span className="ranking-preview1-text24 Content16SemiBold">#</span>
          </div>
          <div className="header-second-column">
            <div className="ranking-preview1-group1">
              <div className="ranking-preview1-name">
                <span className="ranking-preview1-text25 Labels13CAPS">
                  {props.text6}
                </span>
              </div>
              <div className="ranking-preview1-volume">
                <span className="ranking-preview1-text26 Labels13CAPS">
                  Volume
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group2">
              <div className="ranking-preview1-change">
                <span className="ranking-preview1-text27 Labels13CAPS">
                  {props.text8}
                </span>
              </div>
              <div className="ranking-preview1-floor">
                <span className="ranking-preview1-text28 Content16SemiBold">
                  {props.text9}
                </span>
                <span className="ranking-preview1-text29 Content16SemiBold">
                  {props.text10}
                </span>
              </div>
              <div className="ranking-preview1-change01">
                <span className="ranking-preview1-text30 Content16SemiBold">
                  {props.text11}
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group-3">
              <div className="ranking-preview1-owners">
                <span className="ranking-preview1-text31">{props.text12}</span>
              </div>
              <div className="ranking-preview1-listed">
                <span className="ranking-preview1-text32 Content16SemiBold">
                  Listed
                </span>
              </div>
              <div className="ranking-preview1-follow">
                <span className="ranking-preview1-text33 Content16SemiBold">
                  {props.text14}
                </span>
              </div>
            </div>
          </div>
        </div>
        <div className="table-row">
          <div className="table-row-first-column">
            <span className="ranking-preview1-text34">{props.text}</span>
            <img
              src="https://aheioqhobo.cloudimg.io/v7/_playground-bucket-v2.teleporthq.io_/946112a0-4ddc-451c-9806-da7178602435/38a63d98-e63d-4f10-ad60-29e0154a8e0a?org_if_sml=15208"
              className="ranking-preview1-image"
            />
          </div>
          <div className="table-row-second-column">
            <div className="ranking-preview1-group11">
              <div className="ranking-preview1-name1">
                <span className="ranking-preview1-text35 Content16SemiBold">
                  Micro Astronauts
                </span>
              </div>
              <div className="ranking-preview1-volume1">
                <span className="ranking-preview1-text36 Content16SemiBold">
                  182 M XRD
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group21">
              <div className="ranking-preview1-change02">
                <span className="ranking-preview1-text37 Content16SemiBold">
                  +31%
                </span>
              </div>
              <div className="ranking-preview1-floor1">
                <span className="ranking-preview1-text-mobile Content16SemiBold">
                  {props.textMobile}
                </span>
                <span className="ranking-preview1-text38 Content16SemiBold">
                  12645 XRD
                </span>
              </div>
              <div className="ranking-preview1-change03">
                <span className="ranking-preview1-text39">{props.text19}</span>
                <span className="ranking-preview1-change-mobile">
                  {props.changeMobile}
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group-31">
              <div className="ranking-preview1-owners1">
                <span className="ranking-preview1-text40 Content16SemiBold">
                  {props.text20}
                </span>
              </div>
              <div className="ranking-preview1-listed1">
                <span className="ranking-preview1-text41 Content16SemiBold">
                  {props.text21}
                </span>
              </div>
              <div className="ranking-preview1-follow1">
                <svg
                  viewBox="0 0 1024 1024"
                  className="ranking-preview1-icon02"
                >
                  <path
                    d="M719.147 894.549c-9.557 0-18.901-2.133-27.563-6.229-1.493-0.597-2.944-1.323-4.437-2.176l-175.232-99.968-175.232 100.011c-1.493 0.853-2.987 1.579-4.523 2.261-20.821 9.728-46.293 7.424-64.896-6.101-20.011-14.507-29.909-39.808-25.003-64.384l40.832-198.059c-15.957-14.507-41.045-37.333-68.352-62.165l-80.853-73.6c-1.152-1.067-2.347-2.261-3.328-3.499-16-16.896-21.717-41.387-14.507-63.659 7.381-22.357 26.453-38.912 49.493-43.051 1.451-0.341 2.859-0.597 4.352-0.768l200.576-22.229 83.029-183.893c0.725-1.621 1.536-3.2 2.56-4.693 11.179-20.181 32.597-32.896 55.851-32.896 23.168 0 44.544 12.715 55.851 32.853 0.896 1.536 1.749 3.115 2.56 4.779l83.115 183.979 200.576 22.229c1.451 0.128 2.901 0.384 4.437 0.725 22.997 4.267 42.069 20.736 49.408 43.179 7.253 22.229 1.493 46.848-14.507 63.744-1.024 1.109-2.176 2.304-3.328 3.328l-149.248 135.851 40.832 197.632c4.821 25.045-5.035 50.261-25.173 64.811-10.752 7.765-23.723 11.989-37.291 11.989zM371.456 614.485c-1.664 7.765-19.883 95.829-36.053 174.165l155.435-88.619c13.099-7.467 29.184-7.467 42.24 0l155.435 88.533-36.224-175.104c-3.029-14.763 1.92-30.080 13.141-40.192l132.267-120.405-177.835-19.669c-14.976-1.664-27.904-11.093-34.176-24.917l-73.728-163.029-73.643 163.029c-6.229 13.739-19.2 23.168-34.176 24.917l-177.835 19.669 132.267 120.405c11.605 10.496 16.384 26.325 12.885 41.216z"
                    className=""
                  ></path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div className="table-row">
          <div className="table-row-first-column">
            <span className="ranking-preview1-text42">
              <span className="ranking-preview1-text43">2</span>
              <br className=""></br>
            </span>
            <img
              src="https://aheioqhobo.cloudimg.io/v7/_playground-bucket-v2.teleporthq.io_/946112a0-4ddc-451c-9806-da7178602435/e6c2d3bf-e5f6-4228-9301-79d2b1020f82?org_if_sml=15304"
              className="ranking-preview1-image1"
            />
          </div>
          <div className="table-row-second-column">
            <div className="ranking-preview1-group12">
              <div className="ranking-preview1-name2">
                <span className="ranking-preview1-text45 Content16SemiBold">
                  {props.text22}
                </span>
              </div>
              <div className="ranking-preview1-volume2">
                <span className="ranking-preview1-text46 Content16SemiBold">
                  379 K XRD
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group22">
              <div className="ranking-preview1-change04">
                <span className="ranking-preview1-text47 Content16SemiBold">
                  {props.text24}
                </span>
              </div>
              <div className="ranking-preview1-floor2">
                <span className="ranking-preview1-text-mobile1 Content16SemiBold">
                  Floor:
                </span>
                <span className="ranking-preview1-text48 Content16SemiBold">
                  {props.text25}
                </span>
              </div>
              <div className="ranking-preview1-change05">
                <span className="ranking-preview1-text49">{props.text26}</span>
                <span className="ranking-preview1-change-mobile1">
                  {props.changeMobile1}
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group-32">
              <div className="ranking-preview1-owners2">
                <span className="ranking-preview1-text50 Content16SemiBold">
                  {props.text27}
                </span>
              </div>
              <div className="ranking-preview1-listed2">
                <span className="ranking-preview1-text51 Content16SemiBold">
                  12%
                </span>
              </div>
              <div className="ranking-preview1-follow2">
                <svg
                  viewBox="0 0 1024 1024"
                  className="ranking-preview1-icon04"
                >
                  <path
                    d="M399.445 390.741c0 0-134.827 14.933-224.768 24.917-8.107 0.981-15.275 6.4-17.963 14.635s0 16.811 5.973 22.229c66.816 60.971 167.211 152.277 167.211 152.277-0.085 0-27.563 132.821-45.824 221.44-1.536 8.021 1.365 16.512 8.363 21.589 6.955 5.077 15.915 5.163 22.955 1.195 78.677-44.715 196.523-111.957 196.523-111.957s117.888 67.243 196.437 112c7.168 3.925 16.128 3.84 23.083-1.237 6.997-5.077 9.899-13.568 8.32-21.547-18.261-88.661-45.696-221.483-45.696-221.483s100.395-91.307 167.211-152.149c5.973-5.589 8.619-14.165 5.973-22.357s-9.813-13.611-17.92-14.549c-89.941-10.069-224.811-25.003-224.811-25.003s-55.893-123.648-93.141-206.080c-3.499-7.381-10.837-12.544-19.456-12.544s-16 5.205-19.328 12.544c-37.291 82.432-93.141 206.080-93.141 206.080z"
                    className=""
                  ></path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div className="table-row">
          <div className="table-row-first-column">
            <span className="ranking-preview1-text52">{props.text1}</span>
            <img
              src="data:image/webp;base64,"
              className="ranking-preview1-image2"
            />
          </div>
          <div className="table-row-second-column">
            <div className="ranking-preview1-group13">
              <div className="ranking-preview1-name3">
                <span className="ranking-preview1-text53 Content16SemiBold">
                  {props.text29}
                </span>
              </div>
              <div className="ranking-preview1-volume3">
                <span className="ranking-preview1-text54 Content16SemiBold">
                  {props.text30}
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group23">
              <div className="ranking-preview1-change06">
                <span className="ranking-preview1-text55 Content16SemiBold">
                  +31%
                </span>
              </div>
              <div className="ranking-preview1-floor3">
                <span className="ranking-preview1-text-mobile2 Content16SemiBold">
                  {props.textMobile2}
                </span>
                <span className="ranking-preview1-text56 Content16SemiBold">
                  1245 XRD
                </span>
              </div>
              <div className="ranking-preview1-change07">
                <span className="ranking-preview1-text57">{props.text33}</span>
                <span className="ranking-preview1-change-mobile2">
                  {props.changeMobile2}
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group-33">
              <div className="ranking-preview1-owners3">
                <span className="ranking-preview1-text58 Content16SemiBold">
                  {props.text34}
                </span>
              </div>
              <div className="ranking-preview1-listed3">
                <span className="ranking-preview1-text59 Content16SemiBold">
                  {props.text35}
                </span>
              </div>
              <div className="ranking-preview1-follow3">
                <svg
                  viewBox="0 0 1024 1024"
                  className="ranking-preview1-icon06"
                >
                  <path
                    d="M719.147 894.549c-9.557 0-18.901-2.133-27.563-6.229-1.493-0.597-2.944-1.323-4.437-2.176l-175.232-99.968-175.232 100.011c-1.493 0.853-2.987 1.579-4.523 2.261-20.821 9.728-46.293 7.424-64.896-6.101-20.011-14.507-29.909-39.808-25.003-64.384l40.832-198.059c-15.957-14.507-41.045-37.333-68.352-62.165l-80.853-73.6c-1.152-1.067-2.347-2.261-3.328-3.499-16-16.896-21.717-41.387-14.507-63.659 7.381-22.357 26.453-38.912 49.493-43.051 1.451-0.341 2.859-0.597 4.352-0.768l200.576-22.229 83.029-183.893c0.725-1.621 1.536-3.2 2.56-4.693 11.179-20.181 32.597-32.896 55.851-32.896 23.168 0 44.544 12.715 55.851 32.853 0.896 1.536 1.749 3.115 2.56 4.779l83.115 183.979 200.576 22.229c1.451 0.128 2.901 0.384 4.437 0.725 22.997 4.267 42.069 20.736 49.408 43.179 7.253 22.229 1.493 46.848-14.507 63.744-1.024 1.109-2.176 2.304-3.328 3.328l-149.248 135.851 40.832 197.632c4.821 25.045-5.035 50.261-25.173 64.811-10.752 7.765-23.723 11.989-37.291 11.989zM371.456 614.485c-1.664 7.765-19.883 95.829-36.053 174.165l155.435-88.619c13.099-7.467 29.184-7.467 42.24 0l155.435 88.533-36.224-175.104c-3.029-14.763 1.92-30.080 13.141-40.192l132.267-120.405-177.835-19.669c-14.976-1.664-27.904-11.093-34.176-24.917l-73.728-163.029-73.643 163.029c-6.229 13.739-19.2 23.168-34.176 24.917l-177.835 19.669 132.267 120.405c11.605 10.496 16.384 26.325 12.885 41.216z"
                    className=""
                  ></path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div className="table-row">
          <div className="table-row-first-column">
            <span className="ranking-preview1-text60">{props.text2}</span>
            <img
              src="data:image/webp;base64,"
              className="ranking-preview1-image3"
            />
          </div>
          <div className="table-row-second-column">
            <div className="ranking-preview1-group14">
              <div className="ranking-preview1-name4">
                <span className="ranking-preview1-text61 Content16SemiBold">
                  {props.text36}
                </span>
              </div>
              <div className="ranking-preview1-volume4">
                <span className="ranking-preview1-text62 Content16SemiBold">
                  {props.text37}
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group24">
              <div className="ranking-preview1-change08">
                <span className="ranking-preview1-text63 Content16SemiBold">
                  {props.text38}
                </span>
              </div>
              <div className="ranking-preview1-floor4">
                <span className="ranking-preview1-text-mobile3 Content16SemiBold">
                  {props.textMobile3}
                </span>
                <span className="ranking-preview1-text64 Content16SemiBold">
                  1245 XRD
                </span>
              </div>
              <div className="ranking-preview1-change09">
                <span className="ranking-preview1-text65">{props.text40}</span>
                <span className="ranking-preview1-change-mobile3">
                  {props.changeMobile3}
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group-34">
              <div className="ranking-preview1-owners4">
                <span className="ranking-preview1-text66 Content16SemiBold">
                  {props.text41}
                </span>
              </div>
              <div className="ranking-preview1-listed4">
                <span className="ranking-preview1-text67 Content16SemiBold">
                  {props.text42}
                </span>
              </div>
              <div className="ranking-preview1-follow4">
                <svg
                  viewBox="0 0 1024 1024"
                  className="ranking-preview1-icon08"
                >
                  <path
                    d="M719.147 894.549c-9.557 0-18.901-2.133-27.563-6.229-1.493-0.597-2.944-1.323-4.437-2.176l-175.232-99.968-175.232 100.011c-1.493 0.853-2.987 1.579-4.523 2.261-20.821 9.728-46.293 7.424-64.896-6.101-20.011-14.507-29.909-39.808-25.003-64.384l40.832-198.059c-15.957-14.507-41.045-37.333-68.352-62.165l-80.853-73.6c-1.152-1.067-2.347-2.261-3.328-3.499-16-16.896-21.717-41.387-14.507-63.659 7.381-22.357 26.453-38.912 49.493-43.051 1.451-0.341 2.859-0.597 4.352-0.768l200.576-22.229 83.029-183.893c0.725-1.621 1.536-3.2 2.56-4.693 11.179-20.181 32.597-32.896 55.851-32.896 23.168 0 44.544 12.715 55.851 32.853 0.896 1.536 1.749 3.115 2.56 4.779l83.115 183.979 200.576 22.229c1.451 0.128 2.901 0.384 4.437 0.725 22.997 4.267 42.069 20.736 49.408 43.179 7.253 22.229 1.493 46.848-14.507 63.744-1.024 1.109-2.176 2.304-3.328 3.328l-149.248 135.851 40.832 197.632c4.821 25.045-5.035 50.261-25.173 64.811-10.752 7.765-23.723 11.989-37.291 11.989zM371.456 614.485c-1.664 7.765-19.883 95.829-36.053 174.165l155.435-88.619c13.099-7.467 29.184-7.467 42.24 0l155.435 88.533-36.224-175.104c-3.029-14.763 1.92-30.080 13.141-40.192l132.267-120.405-177.835-19.669c-14.976-1.664-27.904-11.093-34.176-24.917l-73.728-163.029-73.643 163.029c-6.229 13.739-19.2 23.168-34.176 24.917l-177.835 19.669 132.267 120.405c11.605 10.496 16.384 26.325 12.885 41.216z"
                    className=""
                  ></path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div className="table-row">
          <div className="table-row-first-column">
            <span className="ranking-preview1-text68">{props.text3}</span>
            <img
              src="https://aheioqhobo.cloudimg.io/v7/_playground-bucket-v2.teleporthq.io_/946112a0-4ddc-451c-9806-da7178602435/17e64974-4b39-458e-aa31-90a36a0011ff?org_if_sml=16566"
              className="ranking-preview1-image4"
            />
          </div>
          <div className="table-row-second-column">
            <div className="ranking-preview1-group15">
              <div className="ranking-preview1-name5">
                <span className="ranking-preview1-text69 Content16SemiBold">
                  Super Blade
                </span>
              </div>
              <div className="ranking-preview1-volume5">
                <span className="ranking-preview1-text70 Content16SemiBold">
                  {props.text44}
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group25">
              <div className="ranking-preview1-change10">
                <span className="ranking-preview1-text71 Content16SemiBold">
                  +31%
                </span>
              </div>
              <div className="ranking-preview1-floor5">
                <span className="ranking-preview1-text-mobile4 Content16SemiBold">
                  {props.textMobile4}
                </span>
                <span className="ranking-preview1-text72 Content16SemiBold">
                  {props.text46}
                </span>
              </div>
              <div className="ranking-preview1-change11">
                <span className="ranking-preview1-text73">15000</span>
                <span className="ranking-preview1-change-mobile4">+31%</span>
              </div>
            </div>
            <div className="ranking-preview1-group-35">
              <div className="ranking-preview1-owners5">
                <span className="ranking-preview1-text74 Content16SemiBold">
                  {props.text48}
                </span>
              </div>
              <div className="ranking-preview1-listed5">
                <span className="ranking-preview1-text75 Content16SemiBold">
                  12%
                </span>
              </div>
              <div className="ranking-preview1-follow5">
                <svg
                  viewBox="0 0 1024 1024"
                  className="ranking-preview1-icon10"
                >
                  <path
                    d="M719.147 894.549c-9.557 0-18.901-2.133-27.563-6.229-1.493-0.597-2.944-1.323-4.437-2.176l-175.232-99.968-175.232 100.011c-1.493 0.853-2.987 1.579-4.523 2.261-20.821 9.728-46.293 7.424-64.896-6.101-20.011-14.507-29.909-39.808-25.003-64.384l40.832-198.059c-15.957-14.507-41.045-37.333-68.352-62.165l-80.853-73.6c-1.152-1.067-2.347-2.261-3.328-3.499-16-16.896-21.717-41.387-14.507-63.659 7.381-22.357 26.453-38.912 49.493-43.051 1.451-0.341 2.859-0.597 4.352-0.768l200.576-22.229 83.029-183.893c0.725-1.621 1.536-3.2 2.56-4.693 11.179-20.181 32.597-32.896 55.851-32.896 23.168 0 44.544 12.715 55.851 32.853 0.896 1.536 1.749 3.115 2.56 4.779l83.115 183.979 200.576 22.229c1.451 0.128 2.901 0.384 4.437 0.725 22.997 4.267 42.069 20.736 49.408 43.179 7.253 22.229 1.493 46.848-14.507 63.744-1.024 1.109-2.176 2.304-3.328 3.328l-149.248 135.851 40.832 197.632c4.821 25.045-5.035 50.261-25.173 64.811-10.752 7.765-23.723 11.989-37.291 11.989zM371.456 614.485c-1.664 7.765-19.883 95.829-36.053 174.165l155.435-88.619c13.099-7.467 29.184-7.467 42.24 0l155.435 88.533-36.224-175.104c-3.029-14.763 1.92-30.080 13.141-40.192l132.267-120.405-177.835-19.669c-14.976-1.664-27.904-11.093-34.176-24.917l-73.728-163.029-73.643 163.029c-6.229 13.739-19.2 23.168-34.176 24.917l-177.835 19.669 132.267 120.405c11.605 10.496 16.384 26.325 12.885 41.216z"
                    className=""
                  ></path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div className="table-row">
          <div className="table-row-first-column">
            <span className="ranking-preview1-text76">{props.text4}</span>
            <img
              src="https://aheioqhobo.cloudimg.io/v7/_playground-bucket-v2.teleporthq.io_/946112a0-4ddc-451c-9806-da7178602435/b5cbd7a8-8801-41ef-a64b-c7c95f915315?org_if_sml=110334"
              className="ranking-preview1-image5"
            />
          </div>
          <div className="table-row-second-column">
            <div className="ranking-preview1-group16">
              <div className="ranking-preview1-name6">
                <span className="ranking-preview1-text77 Content16SemiBold">
                  {props.text50}
                </span>
              </div>
              <div className="ranking-preview1-volume6">
                <span className="ranking-preview1-text78 Content16SemiBold">
                  {props.text51}
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group26">
              <div className="ranking-preview1-change12">
                <span className="ranking-preview1-text79 Content16SemiBold">
                  {props.text52}
                </span>
              </div>
              <div className="ranking-preview1-floor6">
                <span className="ranking-preview1-text-mobile5 Content16SemiBold">
                  Floor:
                </span>
                <span className="ranking-preview1-text80 Content16SemiBold">
                  {props.text53}
                </span>
              </div>
              <div className="ranking-preview1-change13">
                <span className="ranking-preview1-text81">{props.text54}</span>
                <span className="ranking-preview1-change-mobile5">
                  {props.changeMobile5}
                </span>
              </div>
            </div>
            <div className="ranking-preview1-group-36">
              <div className="ranking-preview1-owners6">
                <span className="ranking-preview1-text82 Content16SemiBold">
                  {props.text55}
                </span>
              </div>
              <div className="ranking-preview1-listed6">
                <span className="ranking-preview1-text83 Content16SemiBold">
                  {props.text56}
                </span>
              </div>
              <div className="ranking-preview1-follow6">
                <svg
                  viewBox="0 0 1024 1024"
                  className="ranking-preview1-icon12"
                >
                  <path
                    d="M719.147 894.549c-9.557 0-18.901-2.133-27.563-6.229-1.493-0.597-2.944-1.323-4.437-2.176l-175.232-99.968-175.232 100.011c-1.493 0.853-2.987 1.579-4.523 2.261-20.821 9.728-46.293 7.424-64.896-6.101-20.011-14.507-29.909-39.808-25.003-64.384l40.832-198.059c-15.957-14.507-41.045-37.333-68.352-62.165l-80.853-73.6c-1.152-1.067-2.347-2.261-3.328-3.499-16-16.896-21.717-41.387-14.507-63.659 7.381-22.357 26.453-38.912 49.493-43.051 1.451-0.341 2.859-0.597 4.352-0.768l200.576-22.229 83.029-183.893c0.725-1.621 1.536-3.2 2.56-4.693 11.179-20.181 32.597-32.896 55.851-32.896 23.168 0 44.544 12.715 55.851 32.853 0.896 1.536 1.749 3.115 2.56 4.779l83.115 183.979 200.576 22.229c1.451 0.128 2.901 0.384 4.437 0.725 22.997 4.267 42.069 20.736 49.408 43.179 7.253 22.229 1.493 46.848-14.507 63.744-1.024 1.109-2.176 2.304-3.328 3.328l-149.248 135.851 40.832 197.632c4.821 25.045-5.035 50.261-25.173 64.811-10.752 7.765-23.723 11.989-37.291 11.989zM371.456 614.485c-1.664 7.765-19.883 95.829-36.053 174.165l155.435-88.619c13.099-7.467 29.184-7.467 42.24 0l155.435 88.533-36.224-175.104c-3.029-14.763 1.92-30.080 13.141-40.192l132.267-120.405-177.835-19.669c-14.976-1.664-27.904-11.093-34.176-24.917l-73.728-163.029-73.643 163.029c-6.229 13.739-19.2 23.168-34.176 24.917l-177.835 19.669 132.267 120.405c11.605 10.496 16.384 26.325 12.885 41.216z"
                    className=""
                  ></path>
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

RankingPreview1.defaultProps = {
  text12: 'Owners',
  text11: 'Sales',
  text21: '12%',
  text19: '15000',
  changeMobile5: '+31%',
  text6: 'Collection',
  text56: '12%',
  text25: '4245 XRD',
  text35: '12%',
  text51: '32 K XRD',
  text29: 'Keyboard Samurai',
  text24: '+31%',
  text34: '9478',
  text27: '8637',
  text22: 'Scrypto Ninja',
  text55: '9478',
  text52: '+31%',
  text8: 'Change',
  text26: '8888',
  textMobile3: 'Floor:',
  text36: 'Digi Mecha Ultimate',
  text14: 'Follow',
  text20: '9478',
  text: '1',
  text42: '12%',
  text2: '4',
  changeMobile3: '+31%',
  text48: '9478',
  text44: '43 K XRD',
  text58: 'Art',
  text3: '5',
  text41: '9478',
  text33: '15000',
  textMobile2: 'Floor:',
  text54: '15000',
  text4: '100',
  changeMobile2: '+31%',
  text50: 'Philosophical Pirates',
  textMobile4: 'Floor:',
  changeMobile1: '+31%',
  rootClassName: '',
  text9: 'Floor:',
  textMobile: 'Floor:',
  text30: '245 K XRD',
  changeMobile: '+31%',
  text10: 'Floor',
  text1: '3',
  text59: 'Games',
  text40: '15000',
  text46: '1245 XRD',
  text38: '+31%',
  text37: '45 K XRD',
  text53: '1245 XRD',
}

RankingPreview1.propTypes = {
  text12: PropTypes.string,
  text11: PropTypes.string,
  text21: PropTypes.string,
  text19: PropTypes.string,
  changeMobile5: PropTypes.string,
  text6: PropTypes.string,
  text56: PropTypes.string,
  text25: PropTypes.string,
  text35: PropTypes.string,
  text51: PropTypes.string,
  text29: PropTypes.string,
  text24: PropTypes.string,
  text34: PropTypes.string,
  text27: PropTypes.string,
  text22: PropTypes.string,
  text55: PropTypes.string,
  text52: PropTypes.string,
  text8: PropTypes.string,
  text26: PropTypes.string,
  textMobile3: PropTypes.string,
  text36: PropTypes.string,
  text14: PropTypes.string,
  text20: PropTypes.string,
  text: PropTypes.string,
  text42: PropTypes.string,
  text2: PropTypes.string,
  changeMobile3: PropTypes.string,
  text48: PropTypes.string,
  text44: PropTypes.string,
  text58: PropTypes.string,
  text3: PropTypes.string,
  text41: PropTypes.string,
  text33: PropTypes.string,
  textMobile2: PropTypes.string,
  text54: PropTypes.string,
  text4: PropTypes.string,
  changeMobile2: PropTypes.string,
  text50: PropTypes.string,
  textMobile4: PropTypes.string,
  changeMobile1: PropTypes.string,
  rootClassName: PropTypes.string,
  text9: PropTypes.string,
  textMobile: PropTypes.string,
  text30: PropTypes.string,
  changeMobile: PropTypes.string,
  text10: PropTypes.string,
  text1: PropTypes.string,
  text59: PropTypes.string,
  text40: PropTypes.string,
  text46: PropTypes.string,
  text38: PropTypes.string,
  text37: PropTypes.string,
  text53: PropTypes.string,
}

export default RankingPreview1
